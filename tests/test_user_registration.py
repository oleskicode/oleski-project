...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_label("Enter email").fill("example@example.com") #TODO insert some new unused email here
    page.get_by_role("button", name="Create").click()
    # Create Account Page Checks
    expect(page.get_by_role("main")).to_contain_text("Create your account")
    expect(page.locator("#qa-submit-button-create")).to_match_aria_snapshot("- text: Create")
    expect(page.get_by_role("button", name="Create")).to_be_visible()
    page.get_by_role("button", name="Create").click()
    expect(page.get_by_role("main")).to_contain_text("Your account password has been sent to your email. Be sure to verify your account by clicking on the link in the email")
    page.get_by_text("If you haven't received the").click()
    expect(page.get_by_role("main")).to_contain_text("If you haven't received the email, please check the Spam folder.")
    ...
    expect(page.locator("#qa-user-account-status")).to_match_aria_snapshot("- img \"not active\"\n- text: Inactive")
